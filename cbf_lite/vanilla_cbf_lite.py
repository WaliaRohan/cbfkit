...
# Estimator (e.g., Kalman Filter or simple noisy estimation for this case)
def estimator(x_true):
...
# Simulation loop
for _ in range(T):
    # Get belief from estimator
    x_belief = estimator(x_true)

    # Optimization variables
    u = cp.Variable(2)
    delta = cp.Variable()

    # Compute CLF components based on belief (x_belief)
    V = clf(x_belief)
    L_f_V = 2 * (x_belief - goal)
    gamma = 1.0  # CLF gain

    # Compute CBF components based on belief (x_belief)
    h = cbf(x_belief)
    L_f_h = 2 * (x_belief - obstacle)
    alpha = 1.0  # CBF gain
